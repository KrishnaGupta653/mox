...
# v5-J — _shuf1: reservoir sampling O(1) memory, unbiased
_shuf1() {
  local f="$1"
  [[ -s "$f" ]] || return 1
...
# ── locking (v5-B: stale-steal no longer rmdir's unowned lock) ──────────────
...
# v5-W — graceful EXIT trap: always release locks on shell exit
...
# v5-D — _get_checked: distinguishes "unavailable" from "still loading"
...
# _wait_title <url> [max_iterations]
# Smart wrapper: gives extra initial time for YouTube URLs before polling media-title.
# Fixes "watch?v=ID" being returned before yt-dlp resolves metadata.
_wait_title() {
...
# ── v5-L — Fast search: YouTube Data API v3 / Invidious / yt-dlp fallback ─────
#
# Priority: YouTube Data API v3 (if YOUTUBE_API_KEY set)
#         → Invidious API (if INVIDIOUS_HOST set)
#         → yt-dlp scrape (always available, slowest)
#
...
# _do_search: tries API → Invidious → streaming yt-dlp, caches result
#
# API paths: buffered (already fast, single HTTP round-trip)
...
# ── v5-M — Live async fzf search: opens instantly, results load as you type ──
...
# _fzf_common: top-level helper (zsh nested functions don't capture outer scope)
_fzf_common() {
  local header="$1"
  "$FZF" \
    --height 55% --reverse \
...
# ── desktop notification (Phase 2.14) ─────────────────────────
_notify_track() {
...
# ── v5-E — _clean_url: handles Shorts, SoundCloud, Bandcamp, local files ─────
_clean_url() {
  local url="$1"
...
# ── Auto-DJ: refill queue from Last.fm when empty (v5-R) ─────────────────────
_autodj_check() {
...
# do_replay: replay the last played track from history
do_replay() {
...
# v5-S — do_add_next: insert track immediately after current position
do_add_next() {
  _start
  local url
...
# v5-F + v5-K — do_bar: cursor always restored; single jq pass per frame
do_bar() {
  _need
...
# ── v5-P — Real-time synced lyrics from lrclib.net ─────────────────────────
_lyrics_clean_title() {
  local t="$1"
...
# ── v5-O — Terminal album art via chafa ──────────────────────────────────────
do_art() {
  _need
...
# ── do_uxi — launch bridge server + open web dashboard in browser ─────────
...
# ── v5-T — Bookmarks: named snapshots of queue + position ───────────────────
do_bookmark() {
  _need
  local title; title=$(_get media-title)
  [ -z "$title" ] && { _warn "nothing playing"; return; }
...
# v5-C — _txt_kill_bg_job: PID written atomically BEFORE disown
_txt_kill_bg_job() {
...
# v5-N — Parallel txt resolution: all lines resolved concurrently
do_txt() {
  local arg="${1:-}"
  local resume_flag="${2:-}"
  local chosen_file=""
...
# ── v5-G — do_similar: python3 replaced with jq @uri ─────────────────────────
do_similar() {
  _need
...
# ── v5-H — do_export: CSV injection guard ─────────────────────────────────────
do_export() {
...
# ── v5-X — mox index: scan local music library ─────────────────────────────────
do_index() {
